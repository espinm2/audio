...
void Radiosity::ComputeFormFactors() {
  ...
  printf ("Compute form factors (n = %d) ", num_faces); fflush(stdout);
  int count = 1;
  for (int i = 0; i < num_faces; i++) {
    computeFormFactors(i);
    if (i > count * 0.01 * num_faces) { count++; printf ("."); fflush(stdout); }
  }
  printf (" done\n");
...
void Radiosity::computeFormFactors(int i) {
  ...
  for ( int j = 0; j < num_faces; j++) {
    setFormFactor(i,j,computeFormFactor(i,j));
  }
#else
  for ( int j = 0; j < num_faces; j++) {
    setFormFactor(0);
...
float Radiosity::computeFormFactor(int i, int j) {
  ...
  // cast rays between the two patches
  for (int s = 0; s < args->num_form_factor_samples; s++) {

    glm::vec3 point_i, point_j;
    if (args->num_form_factor_samples == 1) {
      point_i = face_i->computeCentroid();
      point_j = face_j->computeCentroid();
    } else {
      point_i = face_i->RandomPoint();
      point_j = face_j->RandomPoint();
    }

    // point to point form factor
    glm::vec3 direction = point_j - point_i;
    glm::vec3 norm_i = face_i->computeNormal();
    glm::vec3 norm_j = face_j->computeNormal();
    float distance = glm::length(direction);
    if (distance < 0.0001) continue;
    direction = glm::normalize(direction);

    // compute visibility term along the ray
    if (args->num_shadow_samples > 0) {
      Ray r = Ray(point_i, direction);
      Hit h = Hit();
      bool intersect = raytracer->CastRay(r,h, true);
      if (intersect && h.getT() < distance-EPSILON)
        continue;
    }

    // finish computing the point to point form factor
    float cos_theta_i = glm::dot(direction,norm_i);
    float cos_theta_j = -glm::dot(direction,norm_j);
    if (cos_theta_i < 0 || cos_theta_j  < 0) continue;

    // point to point form factor (hanrahan formula?)
    factor += cos_theta_i * cos_theta_j * face_j->getArea() / (M_PI * distance * distance + face_i->getArea()); 
  }
  factor /= float(args->num_form_factor_samples);
  return factor;
}
// END SOLUTION
...
float Radiosity::Iterate() {
  ...
  // BEGIN SOLUTION
  int i = max_undistributed_patch;
  glm::vec3 undist = getUndistributed(i) * float(getArea(i));
  setUndistributed(i, glm::vec3(0,0,0));
  
  for ( int j = 0; j < num_faces; j++) {
    float fij = getFormFactor(i,j);
    glm::vec3 contribution = (fij * undist);
    contribution *= 1 / getArea(j);
    glm::vec3 rad = contribution * mesh->getFace(j)->getMaterial()->getDiffuseColor();
    glm::vec3 abs = contribution - rad;
    setUndistributed(j,getUndistributed(j)+rad);
    setRadiance(j,getRadiance(j)+rad);
    setAbsorbed(j,getAbsorbed(j)+abs);
  }

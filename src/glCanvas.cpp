...
glm::vec3 GLCanvas::TraceRay(double i, double j) {
  ...
  if (args->num_antialias_samples == 1) {
    ...
    // for antialiasing, you'll want to trace multiple rays through the
    // pixel and return the average color of all those answers
    assert (args->num_antialias_samples > 1);
    for (int a = 0; a < args->num_antialias_samples; ++a) {
      // Here's what we do with a single sample per pixel:
      // construct & trace a ray through the center of the pixle
      double rand_x = GLOBAL_MTRAND.rand();
      double rand_y = GLOBAL_MTRAND.rand();
      double x = (i+rand_x-args->width/2.0)/double(max_d)+0.5;
      double y = (j+rand_y-args->height/2.0)/double(max_d)+0.5;
      Ray r = camera->generateRay(x,y); //Vec2f(x,y));
      Hit hit;
      color += raytracer->TraceRay(r,hit,args->num_bounces);
      // add that ray for visualization
      RayTree::AddMainSegment(r,0,hit.getT());
    }
    color /= args->num_antialias_samples;
  }
#endif
  // END SOLUTION
